...
filter_length = 64
num_filters_range = [4, 8, 16, 32, 64, 128]
...
for result in results:
    num_filters_params.append(result[3])  # Number of parameters
    ser_values.append(result[2])  # SER

# Plotting results
plt.figure(figsize=(12, 8))
plt.plot(num_filters_params, ser_values, marker='o', linestyle='-')
for i, txt in enumerate(results):
    plt.annotate(f"NF={txt[1]}", (num_filters_params[i], ser_values[i]))

plt.xlabel("Number of Parameters")
plt.ylabel("Symbol Error Rate (SER)")
plt.yscale("log")
plt.title("SER vs. Number of Parameters for Different Numbers of Filters (Averaged over 5 runs)")
plt.grid(True)
plt.show()